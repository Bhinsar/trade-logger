...
export const createTradeSchema = yup.object({
  ...
  entry_time: yup.date().typeError("Entry time must be a valid date").required("Entry time is required"),
  exit_time: yup.date()
  ...
  trade_image_url: yup.array().of(yup.string().url("Must be a valid URL").required()).nullable().optional(),
